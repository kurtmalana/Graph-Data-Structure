...
public:
  ...
  */
    ...
    static void makeCityMap(unordered_map<string, CityNode> &cityGraph,
                                vector<string> cities);
                                
    // makeAdjacencyGraph populates the adjacency vector
    // of the CityNode using a map
    static void makeAdjacencyGraph(unordered_map<string, CityNode> &cityGraph, 
                                            vector<pair<string, string>> trainRoutes);
                                            
    // depthFirstSearchReachableCities uses a dfs helper to traverses through cities 
    // in cities vector to populate reachableCities for each city in cityGraph                                        
    static void depthFirstSearchReachableCities(unordered_map<string, CityNode> &cityGraph, 
                                                        vector<string> cities, 
                                                        unordered_map<string, bool> &visitedCities);                                        
    
    // dfs helper of all reachableCities with a starting city.                        
    static void dfsHelper(unordered_map<string, CityNode> &cityGraph, string startingCity,
                                                        vector<string> &reachableCities,
                                                        unordered_map<string, bool> &visitedCities);
